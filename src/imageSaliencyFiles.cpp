/* This code is to generate the saliency scores of the images to their respective files */
//Cpp
#include <iostream>
#include <fstream>
#include <dirent.h>
...
int main(int ac, char* av[])
{
	...
	if(ac<3)
	{
		cout<<"Enter the input folder for the RGB saliency maps and output folder where files have to be written"<<endl;
		return 0;
	}
	string dir_in = av[1];
	string dir_out = av[2];
	...
	for(int k=0; k<list.size(); k++)
	{
		...
		cout<<"The rows of image is "<<image.rows<<" and columns is "<<image.cols<<endl;
